...
class DeclarativeClientFactoryBean<T> implements BeanFactoryAware, FactoryBean<T> {
    ...
    private WebClient findWebClient(Class<?> clazz, ListableBeanFactory beanFactory) {
        ...
        if (qualifierMergedAnnotation.isPresent()) {
            ...
            WebClient wc = BeanFactoryAnnotationUtils.qualifiedBeanOfType(beanFactory, WebClient.class, valueOfQualifier);
        ...
        throw new IllegalStateException("there should be one and only one WebClient instance or one and only one qualified WebClient instance!");
    }
